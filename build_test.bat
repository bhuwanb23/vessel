...
"C:\Program Files\Microsoft Visual Studio\18\Community\Common7\IDE\CommonExtensions\Microsoft\CMake\CMake\bin\cmake.exe" --build build_plan --config Release --target test_curl
if %ERRORLEVEL% neq 0 (
    echo Build failed!
    exit /b 1
)
echo === Running curl smoke test ===
build_plan\bin\test_curl.exe
